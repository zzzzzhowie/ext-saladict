...
- **All behaviour lives in the configurable `systemPrompt`/`prompt`** (nothing hardcoded in the engine). The built-in default `systemPrompt`/`prompt` ship **empty** (`DEFAULT_OPENAI_SYSTEM_PROMPT`/`DEFAULT_OPENAI_PROMPT` in `openai/auth.ts` are `''`); the canonical prompts live in the repo-root **`PROMPT.md`**, which the user pastes into 词典账号 → OpenAI (systemPrompt handles: English word → study card, Chinese word → English equivalents, sentence/paragraph → translate). The engine just sends the user's configured prompts, so every rule is visible/editable in the settings panel. The selection is never truncated; only the **context** is reduced to the sentence around a word (`extractSentence`) to fill `{{sentence}}`. Note: `fillTemplate` substitutes `{{text}}/{{from}}/{{to}}/{{sentence}}` ONLY in the user `prompt`, never in `systemPrompt`.
...
- Prompts: **not migrated.** The defaults ship empty and the user pastes their own from `PROMPT.md`, so there is nothing to restore and a manual edit is never clobbered. (Earlier revisions tried exact-string `LEGACY_OPENAI_*_PROMPTS` matching and a `promptVersion` stamp; both were removed in favour of empty defaults.)
