...
- **Two lookup modes, decided in code** (`isWordOrPhrase` in `engine.ts`), NOT left to the model (small models misclassify): word/phrase → study-card prompt; sentence/paragraph → forced translate-only prompt. The selection is never truncated; only the **context** is reduced to the sentence around a word (`extractSentence`).
